var validate = new function () {
    ...
    this.setFail = function setFail(selector, message, messageSelector) {
        ...
        if (message !== undefined) {
            ...
            $('<span class="wrn" id="error_' + qid + '" data-focus="' + qid + '">' + message + '</span>').insertAfter(selector);
        }
    }
        ...
        if ($.isNumeric(val)) {
            ...
            if (interval !== 'yyyy') {
                ...
                if (val.length === 4) {
                    ...
                    var currentYearFourDigit = parseInt(new Date().getFullYear());
                    ...
                    if (isBlur || $.inArray(val, centuries) === -1) {
                        var isFuture = false;
                        for (var y = currentYearFourDigit; y <= currentYearFourDigit + 10; y++) {
                            if (val === y.toString().substr(2, 2)) {
                                //Future Date
                                isFuture = true;
                                val = y.toString();
                                break;
                        ...
                        //Past Date
                        if (!isFuture) {
                            var userYearTwoDigit = parseInt(val)
                            ...
                            val = userYearFourDigit.toString();
                        }
                        $(selector).val(val);
                        moveNext = true;
                    }
    ...
    this.dateStringValidate = function dateStringValidate(qid, intervalOrder, isMandatory) {
        ...
        if (split.length === 3) {
            ...
            } else if (dateTerms.indexOf(split[0].toLowerCase()) === -1) {
                return this.dateIntervalValidate(qid, undefined, intervalOrder, isMandatory, split[0], '', '');
            } else {
                this.setPass(selector);
                return true;
            }
            ...
            for (var i = 0; i < intervalOrder.length; i++) {
                if (intervalOrder[i].indexOf('M') === 0) {
                    return this.dateIntervalValidate(qid, undefined, intervalOrder, isMandatory, fullYear, split[0], split[1]);
                } else if (intervalOrder[i].indexOf('d') === 0) {
                    return this.dateIntervalValidate(qid, undefined, intervalOrder, isMandatory, fullYear, split[1], split[0]);
                }
            }
        } else {
